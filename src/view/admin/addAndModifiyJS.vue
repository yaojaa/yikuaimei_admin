<template>
    <el-form ref="form" :model="form" label-width="80px" class="adminRole">
        <el-form-item label="角色名称">
            <el-select v-model="form.role_name" placeholder="例：客服-李梅梅">
                <el-option :label="item.role_name" :value="item.role_name" v-for="item in roleList" :key="`${item.role_id}roleList`"></el-option>
            </el-select>
        </el-form-item>
                                    ...
                                    <template slot-scope="scope">
                                        <el-checkbox-group v-model="form.role_power_ids">
                                            <el-checkbox 
                                                :label="chil.power_id"  
                                                name="type" 
                                                v-for="chil in scope.row.children" 
                                                :key="`${chil.power_id}chil`">
                ...
                <el-table-column label="权限分配" >
                    <template slot-scope="scope">
                        <p v-if="scope.row.children && $_poweDeep(scope.row.children) === 2">
                            {{ scope.row.children ? scope.row.children[0].power_cn : ''}}
                            {{ scope.row.children ? scope.row.children[0].power_id : ''}}
                            <el-checkbox-group v-model="form.role_power_ids">
                                <el-checkbox 
                                    :label="scope.row.children[0].power_id" 
                                    name="type" >
                                    显示
                                </el-checkbox>
                            </el-checkbox-group>
...
export default {
    ...
    async created(){
        ...
        if(this.role_id){
            await this.$store.dispatch('adminRole/fetchGetRoleInfo',{role_id:1}) 
