<template>
    <el-form ref="form" :model="form" label-width="80px" id="adminUser">
        <el-form-item label="角色名称">
            <el-select v-model="form.role_id" placeholder="请输入用户名">
                <el-option :label="item.role_name" :value="item.role_id" v-for="item in roleList" :key="item.role_name"></el-option>
            </el-select>
        </el-form-item>
...
export default {
    ...
    methods:{
        onSubmit(){
            ...
            let url = this.modifiy ? 'adminUser/fetchModifiy' : 'adminUser/fetchCreate'
